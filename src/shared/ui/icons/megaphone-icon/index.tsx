import { ComponentProps, Ref, forwardRef } from "react"

const Megaphone = (props: ComponentProps<"svg">, svgRef: Ref<SVGSVGElement>) => {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 73 77"
            aria-hidden={true}
            {...props}
            ref={svgRef}
        >
            <path
                d="M29.1364 19.4858V69.4827C29.1407 70.0201 29.0269 70.55 28.8055 71.0234C28.5842 71.4967 28.2625 71.8983 27.8702 72.1907L24.0014 75.1412C23.6266 75.4288 23.1972 75.6087 22.7493 75.6658C22.3014 75.7229 21.8481 75.6555 21.4276 75.4694C21.0071 75.2832 20.6316 74.9836 20.3328 74.596C20.034 74.2083 19.8206 73.7438 19.7106 73.2416L15.068 51.8201"
                fill="white"
            />
            <path
                d="M29.1364 19.4858V69.4827C29.1407 70.0201 29.0269 70.55 28.8055 71.0234C28.5842 71.4967 28.2625 71.8983 27.8702 72.1907L24.0014 75.1412C23.6266 75.4288 23.1972 75.6087 22.7493 75.6658C22.3014 75.7229 21.8481 75.6555 21.4276 75.4694C21.0071 75.2832 20.6316 74.9836 20.3328 74.596C20.034 74.2083 19.8206 73.7438 19.7106 73.2416L15.068 51.8201"
                stroke="black"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M15.0684 51.82C11.3373 51.82 7.75893 50.1167 5.1206 47.0848C2.48228 44.0529 1.00008 39.9407 1.00008 35.6529C1.00008 31.3651 2.48228 27.2529 5.1206 24.221C7.75893 21.1891 11.3373 19.4858 15.0684 19.4858H29.1368C29.1368 19.4858 48.3049 19.4858 66.7344 1.74232C67.1443 1.35251 67.642 1.10425 68.1697 1.02635C68.6975 0.948452 69.2336 1.0441 69.7159 1.30221C70.1983 1.56032 70.607 1.97029 70.8948 2.48458C71.1826 2.99887 71.3376 3.59639 71.3418 4.20781V67.098C71.3376 67.7094 71.1826 68.3069 70.8948 68.8212C70.607 69.3355 70.1983 69.7455 69.7159 70.0036C69.2336 70.2617 68.6975 70.3574 68.1697 70.2795C67.642 70.2016 67.1443 69.9533 66.7344 69.5635C48.3049 51.82 29.1368 51.82 29.1368 51.82H15.0684Z"
                fill="white"
                stroke="black"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M56.9524 9.65155V61.1516"
                stroke="black"
                strokeWidth="2"
                strokeLinecap="round"
            />
        </svg>
    )
}
export const MegaphoneIcon = forwardRef(Megaphone)
